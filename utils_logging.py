...
from trustworthiness_conditional import *

def filter_valid_data(y_actual, y_pred, y_pred_sm):
    """
...
def save_results(save_path, k, y_actual, y_pred, y_pred_sm, val_dataset, cl, seed_value, dev):
    ...
    y_actual, y_pred, y_pred_argmax, y_pred_sm = filter_valid_data(y_actual, y_pred, y_pred_sm)
    ...
    if val_dataset not in ['JST', 'JNP', 'JKT']:
        ...
        ACC = classification_JIGSAWS(y_actual, y_pred)
    ...
    if val_dataset not in ['JST', 'JNP', 'JKT']:
        NTS = binary_trustworthiness(y_actual, y_pred_sm, cl = cl).main()

    else: NTS = multi_trustworthiness(y_actual, y_pred_sm, cl = cl).main()
        
    if not dev:
        df         = pd.DataFrame({'Seed' : seed_value}, index = [0])
        save_name  = 'Seed.xlsx'
        save_this_location (save_path, save_name, df)
...
def classification_JIGSAWS (y_actual, y_pred):
    """
...
def save_this_location (save_folder, save_name, df):
    """
