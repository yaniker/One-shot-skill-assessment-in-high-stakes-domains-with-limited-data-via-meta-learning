...
def _binary_trustworthiness(y_test: list, y_pred_sm: list, cl: str,
                           alpha: int = 1, beta: int = 1) -> list:
    """
    Defines a function for evaluating binary trustworthiness based on predicted softmax probabilities and actual binary outcomes.
    ...
    oracle = np.array(y_test)
    actor_response = np.argmax(y_pred_sm, axis=-1)
    actor_confidence = np.array(y_pred_sm)
    ...
    adjusted_confidence = np.where(correct_predictions[:, np.newaxis],
                                    actor_confidence * alpha,
                                    (1 - actor_confidence) * beta)
    ...
    labels = ['All ' + cl[0], 'True Negatives', 'False Positives',
              'All ' + cl[1], 'True Positives', 'False Negatives']

    NTS = [(1 / arg.shape[0]) * np.sum(arg) if arg.size else 'na' for arg in args]
    return NTS
                
class _multi_trustworthiness(y_test: list, y_pred_sm: list,
                            cl: list = ['0', '1', '2'], alpha: int = 1, beta: int = 1)  -> list:
    """
    Defines a function for evaluating multi-class trustworthiness based on predicted softmax probabilities and actual outcomes.
    ...
    TTs = {c: [] for c in cl}  # True Trust scores
    TFs = {f"{i}{j}": [] for i in cl for j in cl if i != j}  # False Trust scores
    TFis = {f"{i}{j}": [] for i in cl for j in cl if i != j}  # Inverse False Trust scores

    for i in range(len(oracle)):
        if oracle[i] == actor_response[i]:  # Correct predictions
            TTs[cl[oracle[i]]].append(alpha * np.max(actor_confidence[i]))
        else:  # Incorrect predictions
            wrong_label = f"{cl[oracle[i]]}{cl[actor_response[i]]}"
            TFs[wrong_label].append(beta * (1 - np.max(actor_confidence[i])))
            TFis[wrong_label].append(beta * np.max(actor_confidence[i]))
